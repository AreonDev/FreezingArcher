...
{
    public sealed class RoachesAI : ArtificialIntelligence, IMessageCreator
    {
            ...
            List<Entity> entitiesNearby)
        {
            ...
            if (maze != null && maze.HasFinished)
            {
                ...
                {
                    var temp = light.PointLightPosition - ownPhysics.RigidBody.Position.ToFreezingArcherVector ();
                ...
                if (player != null)
                {
                    ...
                    bool damage = false;
                    foreach (var roach in roachGroup.Roaches)
                    {
                        if ((player_pos - roach.Position).LengthSquared < 2)
                        {
                            damage = true;
                            break;
                        }
        ...
        public override void SetSpawnPosition (Vector3 playerSpawn, PhysicsComponent ownPhysics, object map, FastRandom rand)
        {
            ...
            if (maze != null)
            {
                ...
                if (!gotit)
                {
                    Logger.Log.AddLogEntry (LogLevel.Severe, "RoachesAI", "Failed to generate spawn position!");
                }
