...
namespace Henge3D.Physics
{
    ...
    public sealed class ContactConstraint : Constraint, IRecyclable
    {
        ...
        internal class Allocator : Pool<ContactConstraint>
        {
            ...
            public Allocator (int capacity, int maxPoints)
                : base (capacity)
            {
        ...
        public override void PreProcess ()
        {
            ...
            for (int i = 0; i < _count; i++)
            {
                ...
                {
                    p.Tangent.Normalize ();
                    Vector3.Multiply (ref p.Tangent, -1, out p.Tangent);
                }
                else
                ...
                if (isWarmStarted)
                {
                    ...
                    if (Math.Abs (p.Impulse) >= Constants.Epsilon)
                    {
                        ...
                        Vector3.Multiply (ref impulse, -1, out impulse);
...
/// </summary>
        ...
        public override void ProcessVelocity ()
        {
            ...
            for (int i = 0; i < _count; i++)
            {
                ...
                va.X -= vb.X;
                va.Y -= vb.Y;
                va.Z -= vb.Z; //Vector3.Subtract(ref va, ref vb, out va);
                ...
                if (Math.Abs (impulseMag) >= Constants.Epsilon)
                {
                    ...
                    impulse.X = _normal.X * impulseMag;
                    impulse.Y = _normal.Y * impulseMag;
                    impulse.Z = _normal.Z * impulseMag; // Vector3.Multiply(ref normal, reactImpulse, out impulse);
                    a.ApplyImpulse (ref impulse, ref p.OffsetA);
                    impulse.X = -impulse.X;
                    impulse.Y = -impulse.Y;
                    impulse.Z = -impulse.Z; // Vector3.Multiply(ref impulse, -1, out impulse);
                    b.ApplyImpulse (ref impulse, ref p.OffsetB);
                }
                ...
                if (p.TangentMass >= Constants.Epsilon)
                {
                    ...
                    impulse.X = p.Tangent.X * impulseMag;
                    impulse.Y = p.Tangent.Y * impulseMag;
                    impulse.Z = p.Tangent.Z * impulseMag; // Vector3.Multiply(ref p.Tangent, tangentImpulse, out impulse);
                    a.ApplyImpulse (ref impulse, ref p.OffsetA);
                    impulse.X = -impulse.X;
                    impulse.Y = -impulse.Y;
                    impulse.Z = -impulse.Z; // Vector3.Multiply(ref impulse, -1, out impulse);
        ...
        public override bool ProcessPosition ()
        {
            ...
            for (int i = (_count > 2 && _count < _points.Length ? _count : _count - 1); i >= 0; i--)
            {
                ...
                if (depth <= this.Manager.LinearErrorTolerance)
                    continue;
